...
class DeleteUserBloc extends Bloc<DeleteUserEvent, DeleteUserState> {
  ...
  StreamSubscription? deleteAccountStream;
  DeleteUserBloc({
    required this.deleteUserUseCase,
    required this.authListenBloc,
    required this.deleteAccountBloc,
  }) : super(DeleteUserInitial()) {
    deleteAccountStream = deleteAccountBloc.stream.listen((state) {
    ...
    on<DeleteUserEvent>((event, emit) async {
      if (event is DeleteUserTappedEvent) {
        ...
        final deleteUser =
            await deleteUserUseCase.call(authListenBloc.state.userModel!.id);
